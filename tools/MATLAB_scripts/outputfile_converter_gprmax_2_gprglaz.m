...
% GPS is the file that contains each position (in m) for each traces. 
% This file is necessesary to process the data by GPRglatz (C. Ogier 17.2.2022) 
...
elseif gpr_format == 3
    ...
    HDR.date                  = ([num2str(date_time(1)), '-' ...
                                  num2str(date_time(2),'%02d'), '-' ...
                                  num2str(date_time(3),'%02d')]);      % Date
...
%     fprintf(fid, 'Receiver Serial#   = %s\r\n', HDR.rx_serial);
    ...
    % GPS file 
    fid = fopen([path HDR.fname '.GPS'], 'w');
    for i = 1 : HDR.num_trac
        fprintf(fid, 'Trace #%i at position %i\r\n', i, i-1);
        fprintf(fid, '$GPRMAX,%0.1f\r\n', HDR.position(i));
